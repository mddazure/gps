...
void main(void)
{
	...
	destinationbase = malloc(sizeof(char) * 50000);
...
*/
	...
	if (strcmp(nmea_id, "$GPGGA")==0)
	{
		...
		printf("pointer dummystring = %u\n", dummystring);
	        printf("GSVHeader %s\n",dummystring);

                NumberOfMessages = atoi(strsep(&string,","));
                printf(" NumberOfMessages %i pointer %u\n",NumberOfMessages, &NumberOfMessages);
	        MessageNumber = atoi(strsep(&string,","));

                printf("MessageNumber %i pointer %u\n",MessageNumber, &MessageNumber);
...
if (SERIALIZE(&destination, &destinationSize, model->Sat1) != CODEFIRST_OK)
	...
	destination[(int) destinationSize-1]='\0';
        ...
        printf("Serialized: %s destination pointer%u\n",destination,destination);

	strcat(iotMessage,destination);

	free(destination);
//	free(destinationSize);
