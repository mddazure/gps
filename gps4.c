...
void main(void)
{
	...
	destination = malloc(sizeof(char) * 500);
	iotMessage = malloc(sizeof(char) * 5000);
        iotMessage[0]='{';
...
*/
			...
			if (strcmp(nmea_id, "$GPGGA")==0)
			{
				...
				printf("nmea_id $GPGSV received %s\n",buffer);
                                string = strdup(buffer);
                                mySat->Sat1.UTCTime = UtcTime;
				printf("mySat->Sat1.UTCTime %s\n",mySat->Sat1.UTCTime);
                                ...
                                if (MessageNumber < NumberOfMessages)
				{
					for(counter=0;counter<4;counter=counter+1)
					{
						...
						mySat->Sat1.UTCTime = UtcTime;
                                		printf("mySat->Sat1.UTCTime %s\n",mySat->Sat1.UTCTime);
		                                mySat->Sat1.El = atoi(strsep(&string,","));
        		                        printf("mySat->Sat1.El %i\n",mySat->Sat1.El);
                		                mySat->Sat1.Az = atoi(strsep(&string,","));
                        		        printf("mySat->Sat1.Az %i\n",mySat->Sat1.Az);
                                		mySat->Sat1.Snr = atoi(strsep(&string,","));
	                                	printf("mySat->Sat1.Snr %i\n",mySat->Sat1.Snr);
