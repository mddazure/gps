...
void main(void)
{
	...
	struct termios options;
        ...
        myGps = CREATE_MODEL_INSTANCE(RaspiNamespace, Gp735Type);
        if (myGps == NULL)
        {
			...
			if (strcmp(nmea_id, "$GPGGA")==0)
			{
				...
				UtcTime = strsep(&string,",");
                        ...
                        if(strcmp(nmea_id,"$GPGSV")==0)
                                ...
                                string = strdup(buffer);
				myGps->gsv.UTCTime = UtcTime;
				printf("myGps->gsv.UTCTime %s\n",myGps->gsv.UTCTime);
...
///void serialize_send (Gp735Type* model)
        ...
        if ((messageHandle = IoTHubMessage_CreateFromByteArray(destination,strlen(destination))) == NULL)
        {
